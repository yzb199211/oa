...
//        xAxis.setAxisLineWidth(5f);
    ...
    private void initData() throws Exception {
        ...
        List<ChartBean.Line> label = new ArrayList<>();

        label.addAll(data.get(0).getList().size() > 10 ? data.get(0).getList().subList(0, 10) : data.get(0).getList());
        //设置x轴显示标签数量  还有一个重载方法第二个参数为布尔值强制设置数量 如果启用会导致绘制点出现偏差
        xAxis.setLabelCount(groupCount, false);
        //格式化x轴标签显示字符
        xAxis.setValueFormatter(new LineValueFormatter(label));
